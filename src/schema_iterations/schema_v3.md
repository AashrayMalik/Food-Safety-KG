...
| `fflo:Adulterant` | `fflo:Microorganism` **[v7]** | Biological contaminant, subtype of `fflo:ContaminantAdulterant` (e.g. Salmonella, E. coli). Justification: 175/5,882 (3.0%) validated triplets — e.g. "Salmonella detectedBy IS 5887 Part 3" — reference microbial content with no formal subtype to map to in v6. |
...
| `fflo:ProcessingAid` **[v7]** | — | Sibling of `fflo:FoodAdditive`, subclass of `fkg:ChemicalIngredient`. FSSAI-distinct category — used during processing, not required in final product declaration. Justification: legally distinct regulatory category, confirmed present in corpus, low volume but high modeling-error risk if conflated with FoodAdditive. |
| `fflo:ObligationType` **[v7]** | — | Controlled vocabulary: `fflo:LabelDeclaration`, `fflo:MarkAffixation`, `fflo:SignatureRequirement`, `fflo:PackagingRequirement`, `fflo:FormFiling`, `fflo:TestingRequirement`, `fflo:ConformanceRequirement`, `fflo:ProductionRequirement`. Justification: 426/5,882 (7.2%) validated triplets use `must*` predicates (mustComplyWith, mustConformTo, mustBeStoredAt, mustContain, mustSpecify, mustBe, mustPassTest...) with no v6 relation to map to — largest single gap by volume. |
| `fkg:Food` | — | Primary food entity (Milk, Mustard Oil, Jaggery, Chilli Powder, Atta…) |
| `fkg:Ingredient` | — | Component ingredient of a food |
| `fkg:ChemicalIngredient` | — | Superclass for `fflo:Adulterant`, `fflo:FoodAdditive`, `fflo:ProcessingAid` |
...
*Note: structurally well-modeled already; FSSAI regulatory corpus under-populates this stage. Not a schema gap — see milk+oil unconstrained run for coverage.*
...
| `fflo:SamplingPlan` **[v7]** | Statistical/procedural plan governing lot size, sample count, and acceptance criteria — distinct from `fso:Sample` (an individual physical sample). Justification: 45/5,882 (0.8%) validated triplets form a self-contained recurring sub-pattern (parameters m/c/M) — e.g. "m = Microbiological limit that separates unsatisfactory from satisfactory in a 2-class sampling plan." |
...
| `fflo:RegulatoryOfficer` **[v7]** | Subclass of `prov:Agent`. An individual acting under a `RegulatoryBody`'s authority (e.g. "Designated Officer," "Food Analyst") — distinct from the body itself. Justification: 375/5,882 (6.4%) validated triplets reference an officer/analyst/inspector role directly (hasDuty, requiresQualification, mustBeSignedBy, sendsReportTo), currently typed inconsistently across extractions (Person / OfficialRole / Role) with no formal class. |
| `fflo:RegulatoryAction` | A public act using power assigned by law |
| `fflo:RegulatoryActionType` **[v7]** | Controlled vocabulary for discretionary regulatory-body powers: `fflo:Seizure`, `fflo:Removal`, `fflo:PenaltyReduction`, `fflo:Authorization`, `fflo:Sealing`. Justification: 8/5,882 (0.1%) validated triplets (mayAdd, maySeal, canSeize, mayTake) plus canIssue in the officer cluster — thin but zero current mapping options. |
...
*Note: hasAcidityLimit / hasAlcoholLimit / hasFatContentLimit / hasFatLimit / hasResidueLimit / hasMaxFatContent / hasPermissibleLimit / hasMaxLimit / hasMaximumLimit / hasMinimumLimit / hasMicrobiologicalLimit — 378/5,882 (6.4%) — are NOT new entities/relations. All decompose into the existing PermissibleLimit --forProperty--> ssn:Property / --hasValue--> fso:Measurement pattern, with the property name (acidity, fat content, residue...) as the ssn:Property node. This is a canonicalization/mapping fix, not a schema gap — see mapping rule below.*
...
| `fflo:hasObligation` **[v7]** | `fflo:FoodBusinessOperator` ∪ `fkg:Food` | `fflo:ObligationType` | Specific compliance obligation, distinct from the coarse `fflo:compliesWith` edge. See ObligationType justification above. |
| `fflo:hasScientificName` **[v7]** | `fkg:Ingredient` ∪ `fkg:Food` | `xsd:string` | Datatype property for taxonomic/scientific naming. Justification: 24/5,882 (0.4%) — e.g. "Black gram (Urd) – Phaseolus mungoLinn" — small, clean, unambiguous pattern. |
...
| `fflo:governsSampling` **[v7]** | `fflo:SamplingPlan` | `fso:SampleType` | Links a sampling plan to the sample classification it governs. See SamplingPlan justification above. |
...
| `fflo:forProperty` | `fflo:PermissibleLimit` | `ssn:Property` | Grounds limit to the measured property. Use for all limit-variant predicates (acidity/alcohol/fat/residue limits) per mapping note above |
...
| `fflo:compliesWith` | `fflo:FoodBusinessOperator` ∪ `fkg:Food` | `fflo:FoodStandard` ∪ `fflo:RegulatoryDocument` | Regulatory compliance obligation (coarse, yes/no). For specific obligation content, use `fflo:hasObligation` [v7]. Do NOT use `fflo:conformsTo` |
...
| `fflo:isEmpoweredTo` **[v7]** | `fflo:RegulatoryBody` | `fflo:RegulatoryActionType` | Discretionary power a regulatory body may exercise. See RegulatoryActionType justification above. |
| `fflo:actsOnBehalfOf` **[v7]** | `fflo:RegulatoryOfficer` | `fflo:RegulatoryBody` | Links an officer to the body they act for. See RegulatoryOfficer justification above. |
